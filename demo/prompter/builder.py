...
def build_static_prompt(spec: TestSpec) -> str:
    # 1. Fetch framework rules
    rules = get_junit_prompt_rules(spec.junit_version, spec.has_mockito)
    java_guidance = get_java_guidance(spec.java_version)
    
    # 2. Format Constructor Guidance
    if spec.constructor_sigs:
        c_lines = [f"To construct the class under test `{spec.class_name}`, use one of the constructors shown in its constructors block:"]
        for sig in spec.constructor_sigs:
            c_lines.append(f"  - `{sig}`")
        constructor_guidance = "\n".join(c_lines)
    else:
        constructor_guidance = f"To construct the class under test `{spec.class_name}`, use its default constructor (no arguments)."
        
    # 3. Format Branch / Logical Guidance
    cf = spec.control_flow_characteristics
    b_lines = ["BRANCH & LOGIC COVERAGE GOALS:"]
    if cf.get("has_loops"):
        b_lines.append("- The target code contains loops. Test boundary states (0, 1, and multiple iterations) of loops.")
    if cf.get("has_conditionals"):
        b_lines.append("- The target code contains conditional paths. Cover all branches (equal, less-than, greater-than, true, false, and boundary checks).")
    if cf.get("has_exceptions"):
        b_lines.append("- The target code throws exceptions. Ensure you include a test verifying that the exception is thrown on invalid/edge inputs.")
    
    if len(b_lines) == 1:
        b_lines.append("- Test regular behavior, edge cases, and boundary values.")
    branch_guidance = "\n".join(b_lines)
    
    # 4. Format Private Method Delegation Guidance
    if spec.private_method_delegation:
        ...
        for m in spec.private_method_delegation:
            p_lines.append(f"  - `{m}`")
        p_lines.append("Since private methods cannot be called directly in unit tests, you MUST test them indirectly. Choose inputs to the public method that cover the internal branches of these private helper methods.")
        private_method_guidance = "\n".join(p_lines)
    else:
        private_method_guidance = ""
        
    # 5. Format Collaborator Strategies Guidance
    ...
    # 6. Format Related Sources Guidance
